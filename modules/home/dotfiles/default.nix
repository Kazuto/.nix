...
in
{
  xdg.configFile = {
    ...
    "zsh/.functions".source = link "${dotfiles}/.config/zsh/.functions";

    # weave has no nix module (not in nixpkgs)
    "weave/config.yaml".text = ''
      branch:
        max_length: 60
  ...
  home.file = {
    ...
    ".hushlogin".source  = link "${dotfiles}/.hushlogin";
  };
}
